...
clicker.double_tap(coords: tuple[int, int], duration: int = 50, gap: int = 100)
# Double-taps at coords in one device-timed action. duration = hold time of
# each tap in ms; gap = pause between taps in ms (0-500).
...
Under the hood `Clicker.click` calls `POST /{device_id}/tap` (combined move+click with a device-settle delay), `double_tap` calls `POST /{device_id}/double-tap` (both taps timed locally by the device), and `swipe` calls `POST /{device_id}/move` (absolute coordinate movement). Action endpoints don't raise on failure — they return `{"success": bool, "message": str}`.
...
double_tap(device_id, coords, duration=50, gap=100)         # POST /{id}/double-tap
