...
```python
screen = get_screen(device: str | Clicker) -> Screen | None
...
# Convenience: execute + poll. Auto-cancels on timeout.
```
...
- **Screen reuse**: fetch once with `screen = get_screen(clicker)`, then call `screen.find()`, `screen.contains()`, `screen.find_and_click()` multiple times. Use the standalone `find_and_click(clicker, ...)` only when you need a fresh screen capture.
